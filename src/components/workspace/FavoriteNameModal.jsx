...
export default function FavoriteNameModal({ isOpen, defaultName, onConfirm, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div 
        className="w-full max-w-sm bg-surface-card border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col pointer-events-auto animate-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="p-4 bg-white/[0.02] border-t border-white/5 flex gap-2 justify-end">
          <button 
            onClick={onCancel}
            className="px-4 py-2 text-[11px] font-bold uppercase tracking-widest text-zinc-500 hover:text-white hover:bg-white/5 rounded-lg transition-colors border border-transparent hover:border-white/10"
          >
            Cancelar
          </button>
          <button 

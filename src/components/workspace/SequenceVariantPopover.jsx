...
/**
 * SequenceVariantPopover — Grid visual com mini-wireframes de cada variante de sequência.
 * Abre acima do botão "Variante".
 */
export default function SequenceVariantPopover({
  currentVariantIndex,
  onSelect,
  onClose,
  brandColor,
    ...
    >
      <div className="bg-zinc-950/95 backdrop-blur-xl border border-zinc-800/80 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] p-4 min-w-[320px] max-w-[380px]">
        ...
        <VariantPopoverHeader
          label="Layouts de Sequência"
          activeLabel={SEQUENCE_VARIANT_META.find(v => v.id === currentVariantIndex)?.name || 'Original'}
          onClose={onClose}
        />

        {/* Grid de variantes com Scroll */}
        <div className="grid grid-cols-4 gap-2.5 max-h-[320px] overflow-y-auto pr-1.5 custom-scrollbar">
          {SEQUENCE_VARIANT_META.map((variant) => (
            <button
              key={variant.id}
              onClick={() => {
                onSelect(variant.id);

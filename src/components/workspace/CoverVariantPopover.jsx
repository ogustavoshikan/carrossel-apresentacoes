...
function VariantThumbnail({ variantId, brandColor, isSelected }) {
  ...
  const layouts = {
    ...
    // 47: Giant Number Base
    47: (
      <div className="w-full h-full flex flex-col relative rounded-[3px] overflow-hidden" style={{ background: '#EBE9E1' }}>
        ...
        <div className="absolute bottom-2 right-2 w-3 h-3 rounded-full bg-black flex items-center justify-center">
          <div className="w-1 h-1 border-t border-r border-white rotate-45 scale-[0.5]" />
        </div>
      </div>
    ),
    // 48: Split Number Dark
    48: (
    ...
    // 50: Massive Typo Frame
    50: (
      <div className="w-full h-full p-1 rounded-[3px] overflow-hidden relative" style={{ background: accent }}>
    ...
    >
      <div className="bg-zinc-950/95 backdrop-blur-xl border border-zinc-800/80 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] p-4 min-w-[280px]">
        ...
        <VariantPopoverHeader
          ...
          activeLabel={currentVariantIndex === 0 ? 'Original' : COVER_VARIANT_META.find(v => v.id === currentVariantIndex)?.name}
          onClose={onClose}
        />

...
import { X, Calendar, Shuffle } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-3 relative">
          <div className="flex items-center gap-2 pr-12">
            ...
            <span className="text-[10px] font-mono text-zinc-600 truncate max-w-[120px]">
              {CTA_VARIANT_META.find(v => v.id === currentVariantIndex)?.title || 'Desconhecida'}
            </span>
          </div>
          <div className="absolute -right-2 -top-2 flex items-center gap-0.5">
            <button 
              onClick={(e) => { 
                e.stopPropagation(); 
                const randomIndex = Math.floor(Math.random() * CTA_VARIANT_META.length);
                onSelect(CTA_VARIANT_META[randomIndex].id); 
              }} 

...
code = code.replace(
  ...
  `<div className="w-full h-full bg-[#E5E5E5] relative overflow-hidden text-black">$1<div className="absolute inset-0 flex flex-col border-8 border-black z-0 pointer-events-none" /><div className="absolute inset-0 flex flex-col z-10"><div className="h-1/2 w-full bg-black p-8 text-white relative flex flex-col justify-end border-b-8 border-black">`
);
// Fix close variant 5: change the final </div></div> to </div></div></div>
code = code.replace(
  /(<ImageBg data=\{data\} className="absolute inset-0" \/>\s*<\/SmartField>\s*<\/div>\s*)(<\/div>\s*\);\s*\})/g,
  `$1</div>$2`
);
...
code = code.replace(
  ...
  `<div className="w-full h-full relative overflow-hidden bg-white">\n      $1\n      <div className="absolute inset-0 p-6 flex flex-col border-8 pointer-events-none" style={{ borderColor: brandColor }} /><div className="absolute inset-0 p-6 flex flex-col pointer-events-none [&>*]:pointer-events-auto">\n      <div className="flex justify-between items-end mb-4 shrink-0 mt-4">`
);
code = code.replace(
  /(<SmartField field="texto_apoio"[\s\S]*?<\/SmartField>\s*)(<\/div>\s*\);\s*\})/g,
  `$1</div>$2`
);
...
code = code.replace(
  ...
  `<div className="w-full h-full relative overflow-hidden bg-[#FAFAFA]">\n      $1\n      <div className="absolute inset-0 p-6 flex flex-col">\n        <div className="absolute top-10 right-[-40px] w-64 h-64 rounded-full opacity-[0.06] z-0 pointer-events-none" style={{ backgroundColor: brandColor }} />\n        <div className="flex-1 flex flex-col z-10 min-h-0 pt-8 w-[85%] pr-4 relative pointer-events-none [&>*]:pointer-events-auto">`
);
code = code.replace(
  /(<SmartField field="tag"[\s\S]*?<\/SmartField>\s*)(<\/div>\s*\);\s*\})/g,
  `$1</div>$2`
);

fs.writeFileSync(file, code);
console.log("Applied structural layouts perfectly.");
